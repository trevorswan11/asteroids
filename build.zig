...
pub fn build(b: *std.Build) void {
    ...
    addRunStep(b, installExe(b, exe_mod, raylib_dep));
    addTestStep(b, null, exe_mod);
    addFmtStep(b);
...
fn installExe(b: *std.Build, exe_mod: *std.Build.Module, raylib_dep: *std.Build.Dependency) *std.Build.Step.Compile {
...
fn addTestStep(b: *std.Build, lib_mod: ?*std.Build.Module, exe_mod: ?*std.Build.Module) void {
    ...
    if (lib_mod) |lm| {
        addLibTestStep(b, lm, test_step);
    }
    if (exe_mod) |em| {
        addExeTestStep(b, em, test_step);
    }
